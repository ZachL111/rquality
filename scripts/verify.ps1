...
powershell -NoProfile -ExecutionPolicy Bypass -File scripts/check-rquality-detail.ps1

Rscript tests/test_domain_review.R

powershell -NoProfile -ExecutionPolicy Bypass -File scripts/check-rquality-properties.ps1
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }

powershell -NoProfile -ExecutionPolicy Bypass -File scripts/check-rquality-golden.ps1
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }

powershell -NoProfile -ExecutionPolicy Bypass -File scripts/check-rquality-casebook.ps1
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
